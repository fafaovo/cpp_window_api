...
BOOL CTmpFolderCleanerDlg::OnInitDialog()
{
	...
	// TODO:  在此添加额外的初始化代码
	m_list_1.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_CHECKBOXES);
	m_list_1.InsertColumn(0, _T("ID"), 0, 40 );
	m_list_1.InsertColumn(1, _T("零时目录"), 0, 700);

	m_list_2.SetExtendedStyle(LVS_EX_FULLROWSELECT | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES | LVS_EX_CHECKBOXES);
	m_list_2.InsertColumn(0, _T("ID"), 0, 40);
	m_list_2.InsertColumn(1, _T("文件路径"), 0, 640);
	m_list_2.InsertColumn(2, _T("删除状态"), 0, 100);
